defmodule PlateSlate.Menu do
  ...
  defp filter_with(query, filter) do
    Enum.reduce(filter, query, fn
      ...
      {:description, description}, query -> from q in query, where: ilike(q.description, ^"%#{description}%")
      ...
      {:category, category_name}, query -> from q in query, join: c in assoc(q, :category), where: ilike(c.name, ^"%#{category_name}%")
      {:tag, tag_name}, query -> from q in query, join: t in assoc(q, :tags), where: ilike(t.name, ^"%#{tag_name}%")
    end)
  end
  ...
  defp search_ecto(ecto_schema, pattern) do
    Repo.all from q in ecto_schema, where: ilike(q.name, ^pattern) or ilike(q.description, ^pattern)
  end
