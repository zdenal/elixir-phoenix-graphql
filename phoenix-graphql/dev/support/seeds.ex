...
defmodule PlateSlate.Seeds do
  ...
  def run() do
    alias PlateSlate.{Menu, Repo, Accounts}

    %Accounts.User{email: "zdenko@email.com", password: "abcd", role: "employee", name: "Zdenko"}
    |> Repo.insert!
    ...
    PlateSlate.Ordering.create_order(%{items: [%{menu_item_id: water.id, quantity: 4}]})
    PlateSlate.Ordering.create_order(%{items: [%{menu_item_id: water.id, quantity: 1}]})
